...
public class ItemRestController {
	...
	public Item find(@PathVariable String itemUUIDorId) {
		...
		if(StringUtils.isNumeric(itemUUIDorId)) {
			return this.itemRepository.findbyId(Integer.parseInt(itemUUIDorId)).get();	
		} else {
			return this.itemRepository.findbyUUID(UUID.fromString(itemUUIDorId)).get();			
		}
	}
	...
	public ResponseEntity<?> add(@PathVariable String uuid, @RequestBody Item item) {
		...
		Collection collection = this.collectionRepository.findbyUUID(UUID.fromString(uuid)).get();
	...
	private void validateItem(String itemUUIDorId) {		
		if(StringUtils.isNumeric(itemUUIDorId)) {
			this.itemRepository.findbyId(Integer.parseInt(itemUUIDorId)).orElseThrow(() -> new DSONotFoundException(itemUUIDorId));	
		} else {
			this.itemRepository.findbyUUID(UUID.fromString(itemUUIDorId)).orElseThrow(() -> new DSONotFoundException(itemUUIDorId));			
		}
	}
	
	private void validateCollection(String uuid) {		
		this.collectionRepository.findbyUUID(UUID.fromString(uuid)).orElseThrow(() -> new DSONotFoundException(uuid));			
	}
}
