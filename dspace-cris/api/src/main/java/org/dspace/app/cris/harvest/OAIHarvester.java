...
import org.dspace.app.cris.crosswalk.CerifIngestionCrosswalk;
...
public class OAIHarvester {
	...
	public static final String CRISOU_TYPE_ORGUNIT = "org_unit";
	public static final String HARVESTSOURCEID_FIELD_NAME = "harvestsourceid";
	...
	// Set in dspace.cfg as harvester.oai.oreSerializationFormat.{ORESerialKey}
	// = {ORESerialNS}
	private Namespace ORESerialNS;
	private String ORESerialKey;

	// Namespace of the descriptive metadata that should be harvested in
	// addition to the ORE
	// Set in dspace.cfg as harvester.oai.metadataformats.{MetadataKey} =
	// {MetadataNS},{Display Name}
	...
	public OAIHarvester(Context c, DSpaceObject dso, HarvestedCollection hc) throws HarvestingException, SQLException {
		...
		// No need to worry about ORESerializationNamespace, this can never be
		// null
		...
		if (metadataNS == null) {
			log.error("No matching metadata namespace found for \"" + metadataKey
					+ "\", see oai.cfg option \"harvester.oai.metadataformats.{MetadataKey} = {MetadataNS},{Display Name}\"");
	...
	public void runHarvest(int limit) throws SQLException, IOException, AuthorizeException {
		...
		// If we have all selected then make sure that we do not include a set
		// filter
		...
		try {
			// obtain the desired descriptive metadata format and verify that
			// the OAI server actually provides it
			// do the same thing for ORE, which should be encoded in Atom and
			// carry its namespace
			...
			// set the status indicating the collection is currently being
			// processed
			...
			log.debug("Harvesting request parameters: listRecords " + oaiSource + " " + fromDate + " " + toDate + " "
			...
			while (listRecords != null) {
				...
				if (records != null && records.size() > 0) {
					log.info("Found " + records.size() + " more records to process (" + totalRecord
							+ " already processed)");
					...
					for (Element record : records) {
						...
						if (HarvestScheduler.interrupt == HarvestScheduler.HARVESTER_INTERRUPT_STOP) {
							throw new HarvestingException("Harvest process for " + targetCollection.getID()
									+ " interrupted by stopping the scheduler.");
						}
						// check for timeout
				...
				try {
					...
					// In case of an exception, make sure to restore our
					// authentication state to the previous state
					ourContext.restoreAuthSystemState();
				}
				ourContext.commit();
			}
		...
		if (((limit > 0) && (effectiveRecord < limit)) || (limit < 0)) {
			// If we got to this point, it means the harvest was completely
			// successful
	 ...
	 * 
	 * @param record
	 *            a JDOM Element containing the actual PMH record with
	 *            descriptive metadata.
	 * @param OREPrefix
	 *            the metadataprefix value used by the remote PMH server to
	 *            disseminate ORE. Only used for collections set up to harvest
	 *            content.
	 */
	private int processRecord(Element record, String OREPrefix) throws SQLException, AuthorizeException, IOException,
			CrosswalkException, HarvestingException, ParserConfigurationException, SAXException, TransformerException {
		WorkspaceItem wi = null;
		Date timeStart = new Date();
		int effectiveRecord = 0;
		// grab the oai identifier
		String oaiID = record.getChild("header", OAI_NS).getChild("identifier", OAI_NS).getText();
		Element header = record.getChild("header", OAI_NS);
		if (("rp".equals(harvestRow.getHarvestMetadataConfig()))
				|| ("ou".equals(harvestRow.getHarvestMetadataConfig()))) {
			return importEntity(record, harvestRow.getHarvestMetadataConfig());
		}
		Item item = HarvestedItem.getItemByOAIId(ourContext, oaiID, targetCollection.getID());
		...
		if (header.getAttribute("status") != null && header.getAttribute("status").getValue().equals("deleted")) {
			log.info("Item " + oaiID + " has been marked as deleted on the OAI server.");
			if (item != null) {
		...
		// If we are only harvesting descriptive metadata, the record should
		// already contain all we need
		List<Element> descMD = record.getChild("metadata", OAI_NS).getChildren();
		IngestionCrosswalk MDxwalk = (IngestionCrosswalk) PluginManager.getNamedPlugin(IngestionCrosswalk.class,
				this.metadataKey);

		if (MDxwalk instanceof CerifIngestionCrosswalk) {
			((CerifIngestionCrosswalk) MDxwalk).setSorurceRef(
		...
		if (harvestRow.getHarvestType() > 1) {
			try {
				oreREM = getMDrecord(harvestRow.getOaiSource(), oaiID, OREPrefix).get(0);
				ORExwalk = (IngestionCrosswalk) PluginManager.getNamedPlugin(IngestionCrosswalk.class,
						this.ORESerialKey);
			} catch (Exception e) {
				log.error("Error generating oreREM for item " + oaiID);
			}
		}
		...
		{
			log.debug("Item " + item.getHandle() + " was found locally. Using it to harvest " + oaiID + ".");

			// FIXME: check for null pointer if for some odd reason we don't
			// have a matching hi
			hi = HarvestedItem.find(ourContext, item.getID());

			// Compare last-harvest on the item versus the last time the item
			// was updated on the OAI provider side
			// If ours is more recent, forgo this item, since it's probably a
			// left-over from a previous harvesting attempt
			...
			if (itemLastHarvest != null && OAIDatestamp.before(itemLastHarvest)) {
				log.info("Item " + item.getHandle()
						+ " was harvested more recently than the last update time reporetd by the OAI server; skipping.");
				return effectiveRecord;
			}
		...
		// NOTE: did not find, so we create (presumably, there will never be a
		// case where an item already
		// exists in a harvest collection but does not have an OAI_id)
		{
			wi = WorkspaceItem.create(ourContext, targetCollection, false);
			item = wi.getItem();

			hi = HarvestedItem.create(ourContext, item.getID(), oaiID);
			...
			if (handle != null) {
				...
				if (dso != null) {
					log.warn("Handle collision: attempted to re-assign handle '" + handle
							+ "' to an incoming harvested item '" + hi.getOaiID() + "'.");
					wi.deleteWrapper();
					return effectiveRecord;
				}
			}
		...
		String provenanceMsg = "Item created via OAI harvest from source: " + this.harvestRow.getOaiSource() + " on "
				+ new DCDate(hi.getHarvestDate()) + " (GMT).  Item's OAI Record identifier: " + hi.getOaiID();
		...
		log.info("Item " + item.getHandle() + "(" + item.getID() + ")" + " has been ingested. The whole process took: "
				+ timeTaken + " ms. ");
		return effectiveRecord;
	}
	 ...
	 * 
	 * @param item
	 *            a DSpace item recently pushed through an ingestion crosswalk
	...
	private void scrubMetadata(Item item) throws SQLException, HarvestingException, AuthorizeException, IOException {
		...
		for (Metadatum value : values) {
			...
			if (mdSchema == null && !clearList.contains(value.schema)) {
				// add a new schema, giving it a namespace of "unknown".
				...
				// ignore the offending schema, quietly dropping all of its
				// metadata elements before they clog our gears
				...
				// otherwise, go ahead and generate the error
					...
					throw new HarvestingException(
							"The '" + value.schema + "' schema has not been defined in this DSpace instance. ");
				}
			}

			if (mdSchema != null) {
				// Verify that the element exists; this part is reachable only
				// if the metadata schema is valid
				MetadataField mdField = MetadataField.findByElement(ourContext, mdSchema.getSchemaID(), value.element,
						value.qualifier);
				if (mdField == null) {
					if (fieldChoice.equals("add")) {
						...
						throw new HarvestingException("The '" + value.element + "." + value.qualifier
	 ...
	 *            source Date
	 * @param secondsPad
	 *            number of seconds to subtract from the date
	 * @return a string in the format 'yyyy-mm-ddThh:mm:ssZ' and converted to
	 *         UTC timezone
	 */
	 ...
	 * @param MDNamespace
	 *            the namespace that we are trying to resolve to the
	 *            metadataPrefix
	 ...
	 * 
	 * @param status
	 *            the current status of the collection, usually
	 *            HarvestedCollection.STATUS_OAI_ERROR or
	 *            HarvestedCollection.STATUS_UNKNOWN_ERROR
	 * @param ex
	 *            the Exception that prompted this action
	 */
	 ...
	 *            the address of the OAI-PMH provider
	 * @param itemOaiId
	 *            the OAI identifier of the target item
	 * @param metadataPrefix
	 *            the OAI metadataPrefix of the desired metadata
	 ...
	 * @param metaPrefix
	 * @param testORE
	 *            whether the method should also check the PMH provider for ORE
	 *            support
	 * @return list of errors encountered during verification. Empty list
	 *         indicates a "success" condition.
	 */
	public static List<String> verifyOAIharvester(String oaiSource, String oaiSetId, String metaPrefix,
			boolean testORE) {
		...
		// Next, make sure the metadata we need is supported by the target
		// server
		...
		try {
			...
			errorSet.add(OAI_ADDRESS_ERROR + ": OAI did not respond to ListMetadataFormats query  ("
					+ ORE_NS.getPrefix() + ":" + OREOAIPrefix + " ; " + DMD_NS.getPrefix() + ":" + DMDOAIPrefix + "):  "
					+ ex.getMessage());
			return errorSet;
		}
		...
		try {
			...
			if (!"all".equals(oaiSetId)) {
				...
				if (ls.getErrors() != null && ls.getErrors().getLength() > 0) {
					...
					// Check if we can find items, if so this indicates that we
					// have children and our sets exist
		...
		private void scheduleLoop() {
			...
			while (true) {
				try {
					...
					// Stage #1: if something is ready for harvest, push it onto
					// the ready stack, mark it as "queued"
					mainContext = new Context();
					...
					// Stage #2: start up all the threads currently in the queue
					...
					// Finally, wait for the last few remaining threads to
					// finish
					// TODO: this step might be unnecessary. Theoretically a
					// single very long harvest process
					// could then lock out all the other ones from starting on
					// their next iteration.
					// FIXME: also, this might lead to a situation when a single
					// thread getting stuck without
				...
				// Stage #3: figure out how long until the next iteration and
				// wait
				try {
	...
	private int importEntity(Element record, String entityType) {
		String oaiID = record.getChild("header", OAI_NS).getChild("identifier", OAI_NS).getText();
		DSpace dspace = new DSpace();
		ApplicationService applicationService = dspace.getServiceManager().getServiceByName("applicationService",
				ApplicationService.class);
		String source = ConfigurationManager.getProperty("oai", "harvester.collection.fvg." + targetCollection.getID());
		if(StringUtils.isBlank(source)){
			log.error("No source found for collection ID: " + targetCollection.getID());
			return 0;
		}
		switch (entityType) {
			case "rp":
				ResearcherPage rp = HarvestedItem.getRPByOAIId(ourContext, source, oaiID);
				boolean newRP = false;
				if (rp == null) {
					...
					rp.setStatus(true);
					newRP = true;
					log.info("New RP found: " + oaiID);
				}
				try {
					String rpID = record.getChild("metadata", OAI_NS).getChild("CERIF", CERIF_NS)
							.getChild("cfPers", CERIF_NS).getChild("cfPersId", CERIF_NS).getText();
					setStringValueRP(rp, "harvestsourceid", rpID, applicationService);
				} catch (Exception e) {
					log.warn("ID not found. Deleted");
					if(!newRP){
						rp.setStatus(false);
						applicationService.saveOrUpdate(org.dspace.app.cris.model.ResearcherPage.class, rp);
					}
					return 0;
				}
				List<Element> names = record.getChild("metadata", OAI_NS).getChild("CERIF", CERIF_NS)
						.getChild("cfPers", CERIF_NS).getChildren("cfPersName_Pers", CERIF_NS);
				for (int i = 0; i < names.size(); i++) {
					Element name = names.get(i);
					String fullName = "";
					if (name.getChild("cfFamilyNames", CERIF_NS) != null) {
						fullName += name.getChild("cfFamilyNames", CERIF_NS).getText();
					}
					if (name.getChild("cfFirstNames", CERIF_NS) != null) {
						fullName += ", " + name.getChild("cfFirstNames", CERIF_NS).getText();
					}
					if (i == 0) {
						setStringValueRP(rp, "fullName", fullName, applicationService);
						if (name.getChild("cfOtherNames", CERIF_NS) != null) {
							setStringValueRP(rp, "translatedName", name.getChild("cfOtherNames", CERIF_NS).getText(),
									applicationService);
						}
					} else {
						setStringValueRP(rp, "variants", fullName, applicationService);
					}
				}
				List<Element> identifiers = record.getChild("metadata", OAI_NS).getChild("CERIF", CERIF_NS)
						.getChild("cfPers", CERIF_NS).getChildren("cfFedId", CERIF_NS);
				if (identifiers != null) {
					for (Element identifier : identifiers) {
						// ORCID
						if ("ORCID".equals(ClassEnum.fromUuid(
								identifier.getChild("cfFedId_Class", CERIF_NS).getChild("cfClassId", CERIF_NS).getText())
								.getName())) {
							setStringValueRP(rp, "orcid", identifier.getChild("cfFedId", CERIF_NS).getText(),
									applicationService);
							break;
						}
					}
				}
				List<Element> orgUnits = record.getChild("metadata", OAI_NS).getChild("CERIF", CERIF_NS)
						.getChild("cfPers", CERIF_NS).getChildren("cfPers_OrgUnit", CERIF_NS);
				for (Element orgUnit : orgUnits) {
					String orgUnitId = orgUnit.getChild("cfOrgUnitId", CERIF_NS).getText();
					OrganizationUnit ou = HarvestedItem.getOUByRemoteId(ourContext, source, orgUnitId);
					if (ou != null) {
						setOUValueRP(rp, "dept", ou, applicationService);
					}
					break;
				}
				applicationService.saveOrUpdate(org.dspace.app.cris.model.ResearcherPage.class, rp);
				break;
			case "ou":
				OrganizationUnit ou = HarvestedItem.getOUByOAIId(ourContext, source, oaiID);
				boolean newOU = false;
				if (ou == null) {
					ou = new org.dspace.app.cris.model.OrganizationUnit();
					ou.setSourceRef(source);
					ou.setSourceID(oaiID);
				...
				try {
					orgUnitID = record.getChild("metadata", OAI_NS).getChild("CERIF", CERIF_NS)
							.getChild("cfOrgUnit", CERIF_NS).getChild("cfOrgUnitId", CERIF_NS).getText();
					setStringValueOU(ou, "harvestsourceid", orgUnitID, applicationService);
				} catch (Exception e) {
					log.warn("ID not found. Deleted");
					if(!newOU){
						ou.setStatus(false);
						applicationService.saveOrUpdate(org.dspace.app.cris.model.OrganizationUnit.class, ou);
					}
					return 0;
				}
				try {
					Element orgUnit = record.getChild("metadata", OAI_NS).getChild("CERIF", CERIF_NS)
							.getChild("cfOrgUnit", CERIF_NS).getChild("cfName", CERIF_NS);
					if(null!=orgUnit){
						setStringValueOU(ou, "name", orgUnit.getText(), applicationService);
					}
				} catch (Exception e) {
	...
	private void setStringValueOU(org.dspace.app.cris.model.OrganizationUnit crisOU, String key, String value,
			ApplicationService applicationService) {
		OUPropertiesDefinition pdef = applicationService
				.findPropertiesDefinitionByShortName(OUPropertiesDefinition.class, key);
		List<OUProperty> proprietaDellaTipologia = crisOU.getProprietaDellaTipologia(pdef);
		if (StringUtils.isBlank(value)) {
			if (proprietaDellaTipologia != null && proprietaDellaTipologia.size() != 0) {
				for (OUProperty p : proprietaDellaTipologia) {
					crisOU.removeProprieta(p);
				}
			}
		} else {
			OUProperty prop = null;
			if (proprietaDellaTipologia == null || proprietaDellaTipologia.size() == 0) {
				TextValue tv = new TextValue();
				tv.setOggetto(value);
				prop = crisOU.createProprieta(pdef);
				prop.setVisibility(VisibilityConstants.PUBLIC);
				prop.setValue(tv);
			} else {
				prop = proprietaDellaTipologia.get(0);
				prop.getValue().setOggetto(value);
			}
		}
	}

	private void setStringValueRP(org.dspace.app.cris.model.ResearcherPage crisRP, String key, String value,
			ApplicationService applicationService) {
		RPPropertiesDefinition pdef = applicationService
				.findPropertiesDefinitionByShortName(RPPropertiesDefinition.class, key);
		List<RPProperty> proprietaDellaTipologia = crisRP.getProprietaDellaTipologia(pdef);
		if (StringUtils.isBlank(value)) {
			if (proprietaDellaTipologia != null && proprietaDellaTipologia.size() != 0) {
				for (RPProperty p : proprietaDellaTipologia) {
					crisRP.removeProprieta(p);
				}
			}
		} else {
			RPProperty prop = null;
			if (proprietaDellaTipologia == null || proprietaDellaTipologia.size() == 0) {
				TextValue tv = new TextValue();
				tv.setOggetto(value);
				prop = crisRP.createProprieta(pdef);
				prop.setVisibility(VisibilityConstants.PUBLIC);
				prop.setValue(tv);
			} else {
				prop = proprietaDellaTipologia.get(0);
	...
	private void setOUValueRP(org.dspace.app.cris.model.ResearcherPage crisRP, String key, OrganizationUnit value,
			ApplicationService applicationService) {
		RPPropertiesDefinition pdef = applicationService
				.findPropertiesDefinitionByShortName(RPPropertiesDefinition.class, key);
		List<RPProperty> proprietaDellaTipologia = crisRP.getProprietaDellaTipologia(pdef);
