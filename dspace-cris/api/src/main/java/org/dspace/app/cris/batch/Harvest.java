...
import org.dspace.app.cris.harvest.OAIHarvester;
import org.dspace.app.cris.harvest.OAIHarvester.HarvestingException;
...
public class Harvest {
	...
	public static void main(String[] argv) throws Exception {
		...
		options.addOption("P", "purge-all", false, "purge all harvestable collections");
		...
		options.addOption("l", "limit", true, "max num item ingested");
		...
		// Run a single harvest cycle on a collection using saved settings.
			...
			harvester.runHarvest(collection, oaiSetID, eperson, limit);
		}
		...
		// purge all collections that are set up for harvesting (obviously for
		// testing purposes only)
			...
			System.out.println("Purging the following collections (deleting items and resetting harvest status): "
					+ cids.toString());
		...
		// Delete all items in a collection. Useful for testing fresh harvests.
			...
			// TODO: implement this... remove all items and remember to unset
			// "last-harvested" settings
		}
	...
	/*
	 * Resolve the ID into a collection and check to see if its harvesting
	 * options are set. If so, return the collection, if not, bail out.
	 */
	protected Collection resolveCollection(String collectionID) {
		...
		try {
			...
			if (collectionID != null) {
				...
				// database ID
					...
					System.out.println("Looking up by id: " + collectionID + ", parsed as '"
							+ Integer.parseInt(collectionID) + "', " + "in context: " + context);
	...
	protected void configureCollection(String collectionID, int type, String oaiSource, String oaiSetId,
			String mdConfigId) {
	...
	/**
	 * Purges a collection of all harvest-related data and settings. All items
	 * in the collection will be deleted.
	 * 
	 * @param collectionID
	 * @param email
	 */
	protected void purgeCollection(String collectionID, String email) {
		System.out.println(
	...
	/**
	 * Run a single harvest cycle on the specified collection under the
	 * authorization of the supplied EPerson
	 */

	protected void runHarvest(String collectionID, String oaiSetID, String email, int limit) {

		System.out.print("Initializing the harvester... ");
		OAIHarvester harvester = null;
		try {
			Collection collection = resolveCollection(collectionID);
			HarvestedCollection hc = HarvestedCollection.find(context, collection.getID(), oaiSetID);
	...
	/**
	 * Resets harvest_status and harvest_start_time flags for all collections
	 * that have a row in the harvested_collections table
	 */
	protected static void resetHarvesting() {
	...
	/**
	 * Starts up the harvest scheduler. Terminating this process will stop the
	 * scheduler.
	 */
	protected static void startHarvester() {
		try {
	 ...
	 *
	 * @param server
	 *            address of the responder's host.
	 * @param set
	 *            name of an item set.
	 * @param metadataFormat
	 *            local prefix name, or null for "dc".
	 */
	protected static void pingResponder(String server, String set, String metadataFormat) {
		List<String> errors;
		...
		errors = OAIHarvester.verifyOAIharvester(server, set, (null != metadataFormat) ? metadataFormat : "dc", true);
		if (errors.isEmpty())
			System.out.println("OK");
		else {
			for (String error : errors)
