...
public class SetRepositoryHelper {
    ...
    public ListSetsResult getSets(XOAIContext context, int offset, int length) {
        ...
        boolean showOnlyOpeanAireSets = true;
        if("openaire-cris".equals(context.getBaseUrl()) && showOnlyOpeanAireSets) { 
        //VSTODO: parametrizzare "openaire-cris" e  showOnlyOpeanAireSets
	        if (offset < statics.size()) {
	            ...
	            if (length + offset < statics.size()) {
	                ...
	                return new ListSetsResult(true, results,statics.size());
	            } else {
	                log.debug("Offset + length greater or equal than static sets size");
	                for (int i = offset; i < statics.size(); i++)
	                    results.add(statics.get(i));
	                return new ListSetsResult(false, results,statics.size()); 
	            }        	
	        }
	        else return new ListSetsResult(false, results,statics.size());
        }
        else {       	        
	        if (offset < statics.size()) {
	            log.debug("Offset less than static sets size");
