...
public class SubmissionCorrespondanceRestController implements InitializingBean {
    ...
    public void afterPropertiesSet() {
		discoverableEndpointsService.register(this, Arrays.asList(new Link("/api/messages", "messages"),
				new Link("/api/messages/read", "messages-read"), new Link("/api/messages/unread", "messages-unread")));
    }
	
	@RequestMapping(method = { RequestMethod.POST })
	public void create(@RequestParam(required = true) UUID uuid, @RequestParam(required = true) String subject,
			@RequestParam(required = true) String description, HttpServletResponse response, HttpServletRequest request)
    	...
    	for (EPerson ep : epersons) {
	        ...
	        catch (MessagingException e)
	        {
	            log.warn(LogManager.getHeader(context, "notifyOfCorrespondence",
	                    "cannot email user="+ep.getEmail()+"; item_id=" + item.getID()
	                    + ":  " + e.getMessage()));
	        }
        }
    }
