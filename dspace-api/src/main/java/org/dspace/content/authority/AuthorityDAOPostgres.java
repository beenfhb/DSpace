...
public class AuthorityDAOPostgres extends AuthorityDAO {
	...
	private static final String SQL_NUM_AUTHKEY_ISSUED = "select count (distinct authority) as num from item left join metadatavalue on (item.item_id = metadatavalue.resource_id and metadatavalue.resource_type_id=2) where in_archive = true and metadata_field_id in (QUESTION_ARRAY_PLACE_HOLDER) and confidence <> "
	...
	private static final String SQL_NUM_ITEMSISSUED_BYKEY = "select count (distinct item.item_id) as num from item left join metadatavalue on (item.item_id = metadatavalue.resource_id and metadatavalue.resource_type_id=2) where in_archive = true and metadata_field_id in (QUESTION_ARRAY_PLACE_HOLDER) and confidence <> "
	...
	private static final String SQL_ITEMSISSUED_BYKEY_AND_CONFIDENCE = "SELECT item.* FROM item left join metadatavalue on (item.item_id = metadatavalue.resource_id and metadatavalue.resource_type_id=2) WHERE in_archive=true AND metadata_field_id in (QUESTION_ARRAY_PLACE_HOLDER) AND authority = :par0 AND confidence = :par1";
