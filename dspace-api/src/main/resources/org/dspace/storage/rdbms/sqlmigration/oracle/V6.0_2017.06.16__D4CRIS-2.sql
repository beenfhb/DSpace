...
ALTER TABLE cris_metrics ADD resourceid UUID;
...
UPDATE cris_metrics SET resourceid = (SELECT cris_rpage.uuid FROM cris_rpage WHERE cris_metrics.resourceid_legacy_id = cris_rpage.id) WHERE cris_metrics.resourcetypeid = 9;
UPDATE cris_metrics SET resourceid = (SELECT cris_project.uuid FROM cris_project WHERE cris_metrics.resourceid_legacy_id = cris_project.id) WHERE cris_metrics.resourcetypeid = 10;
UPDATE cris_metrics SET resourceid = (SELECT cris_orgunit.uuid FROM cris_orgunit WHERE cris_metrics.resourceid_legacy_id = cris_orgunit.id) WHERE cris_metrics.resourcetypeid = 11;
UPDATE cris_metrics SET resourceid = (SELECT cris_do.uuid FROM cris_do WHERE cris_metrics.resourceid_legacy_id = cris_do.id) WHERE cris_metrics.resourcetypeid > 11;
